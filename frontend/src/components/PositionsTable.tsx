...
export const PositionsTable = () => {
    ...
    const [closingPositionId, setClosingPositionId] = useState<Hex | null>(
        null
    );
    ...
    const fetchPositionIds = useCallback(async () => {
        ...
        try {
            ...
            if (tradingMode === "Private" && userClient) {
                ids = (await userClient.getOpenPositions()).map(
                    (p) => p.position_id
                );
            } else {
    ...
    useEffect(() => {
        if (isClosed && closingPositionId) {
            toast.success(
                `Position ${closingPositionId.slice(0, 8)}... closed!`
            );
            setPositionIds((prev) =>
                prev.filter(
    ...
    return (
        <>
                                ...
                                {positionsData?.map((positionResult, index) => {
                                    ...
                                    if (btcPrice && size > 0n) {
                                        ...
                                        const requiredMargin =
                                            ...
                                            BPS_DIVISOR;
                                        const bufferUSDC =
                                            totalEquity - requiredMargin;
                                        const priceDelta =
                                            (bufferUSDC * PRICE_PRECISION) /
                                            size;
            ...
            <ManageMarginModal
                ...
                onClose={() =>
                    setModalState({ ...modalState, positionId: null })
                }
