...
export const ManageMarginModal = ({
    positionId,
    onClose,
    onSuccess,
    type,
    ...
    const handleSubmit = async () => {
        ...
        if (tradingMode === "Private") {
            ...
            try {
                toast.info("Please sign the message to authorize.", {
                    id: toastId,
                });
                const actionString =
                    type === "add" ? "ADD_MARGIN" : "REMOVE_MARGIN";
                const msgHash = ethers.solidityPackedKeccak256(
                    ["string", "bytes32"],
                    [actionString, positionId]
                );
                ...
                writeContract(
                    {
                        ...
                        functionName:
                            type === "add" ? "addMargin" : "removeMargin",
    ...
    const description = `Modify the margin on your open position (${positionId?.slice(
        0,
        10
    )}...). Mode: ${tradingMode}`;
